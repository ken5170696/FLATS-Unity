...
public class InRoomRoundTimer : MonoBehaviour
{
	...
	public void OnGUI()
	{
		...
		GUILayout.Label(string.Format("elapsed: {0:0.000}", new object[1] { num }));
		GUILayout.Label(string.Format("remaining: {0:0.000}", new object[1] { num2 }));
		GUILayout.Label(string.Format("turn: {0:0}", new object[1] { num3 }));
	...
	{
		SecondsPerTurn = 5;
		TextPos = new Rect(0f, 80f, 150f, 300f);
