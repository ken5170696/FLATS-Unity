...
public class PhotonStatsGui : MonoBehaviour
{
	...
	public void TrafficStatsWindow(int windowID)
	{
		...
		string text = string.Format("Out {0,4} | In {1,4} | Sum {2,4}", new object[3] { trafficStatsGameLevel.TotalOutgoingMessageCount, trafficStatsGameLevel.TotalIncomingMessageCount, trafficStatsGameLevel.TotalMessageCount });
		string text2 = string.Format("{0}sec average:", new object[1] { num });
		string text3 = string.Format("Out {0,4} | In {1,4} | Sum {2,4}", new object[3]
		{
			trafficStatsGameLevel.TotalOutgoingMessageCount / num,
			trafficStatsGameLevel.TotalIncomingMessageCount / num,
			trafficStatsGameLevel.TotalMessageCount / num
		});
	...
	public PhotonStatsGui()
	{
		statsWindowOn = true;
		statsOn = true;
		statsRect = new Rect(0f, 100f, 200f, 50f);
		WindowId = 100;
