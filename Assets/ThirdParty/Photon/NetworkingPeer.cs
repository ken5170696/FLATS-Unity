...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	protected internal string AppVersion
	{
		get
		{
			return string.Format("{0}_{1}", new object[2]
			{
	...
	{
		LobbyStatistics = new List<TypedLobbyInfo>();
		...
		readStream = new PhotonStream(false, null);
		pStream = new PhotonStream(true, null);
		dataPerGroupReliable = new Dictionary<int, ExitGames.Client.Photon.Hashtable>();
		dataPerGroupUnreliable = new Dictionary<int, ExitGames.Client.Photon.Hashtable>();
	...
	private string GetNameServerAddress()
	{
		...
		return string.Format("{0}{1}:{2}", new object[3] { text, "ns.exitgames.com", value });
	}
	...
	protected internal void SetupProtocol(ServerConnection serverType)
	{
        ...
        connectionProtocol = ConnectionProtocol.WebSocketSecure;
        SocketImplementationConfig[ConnectionProtocol.WebSocketSecure] = typeof(ExitGames.Client.Photon.SocketWebTcp);
#else
		...
		if (AuthMode != AuthModeOption.AuthOnceWss || serverType != ServerConnection.NameServer)
		{
			...
			{
				Debug.Log(string.Concat("This Photon3Unity3d.dll only allows UDP. TransportProtocol was: ", base.TransportProtocol, ". SocketImplementation: ", base.SocketImplementation));
			}
			connectionProtocol = ConnectionProtocol.Udp;
		}
		if ((object)SocketImplementationConfig[ConnectionProtocol.Udp] == null)
		{
	...
	public override void Disconnect()
	{
		if (base.PeerState == PeerStateValue.Disconnected)
		{
			if (!PhotonHandler.AppQuits)
			{
				Debug.LogWarning(string.Format("Can't execute Disconnect() while not connected. Nothing changed. State: {0}", new object[1] { State }));
			}
		}
		else
		{
	...
	public void ChangeLocalID(int newID)
	{
		if (LocalPlayer == null)
		{
			Debug.LogWarning(string.Format("LocalPlayer is null or not in mActors! LocalPlayer: {0} mActors==null: {1} newID: {2}", new object[3]
			{
				LocalPlayer,
				mActors == null,
				newID
	...
	private void LeftRoomCleanup()
	{
		...
		bool flag2 = ((CurrentRoom != null) ? CurrentRoom.AutoCleanUp : PhotonNetwork.autoCleanUpPlayerObjects);
		hasSwitchedMC = false;
		CurrentRoom = null;
		mActors = new Dictionary<int, PhotonPlayer>();
		mPlayerListCopy = new PhotonPlayer[0];
	...
	private void HandleEventLeave(int actorID, EventData evLeave)
	{
		...
		if (playerWithId == null)
		{
			Debug.LogError(string.Format("Received event Leave for unknown player ID: {0}", new object[1] { actorID }));
			return;
		}
	...
	public bool SetMasterClient(int nextMasterId)
	{
		...
		hashtable2.Add((byte)248, mMasterClientId);
		ExitGames.Client.Photon.Hashtable expectedProperties = hashtable2;
	...
	public void OnOperationResponse(OperationResponse operationResponse)
	{
		...
		else if (operationResponse.ReturnCode == 32760 || (operationResponse.OperationCode == 226 && operationResponse.ReturnCode == 32758))
		{
		...
		case 231:
			if (operationResponse.ReturnCode != 0)
			{
				...
				{
					Debug.LogError(string.Format("Authentication failed: '{0}' Code: {1}", new object[2] { operationResponse.DebugMessage, operationResponse.ReturnCode }));
				}
				State = ClientState.Disconnecting;
				Disconnect();
			...
			if (Server == ServerConnection.NameServer || Server == ServerConnection.MasterServer)
			{
				if (operationResponse.Parameters.ContainsKey(225))
				{
					...
					if (!string.IsNullOrEmpty(text4))
					{
						...
						if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
						{
							DebugReturn(DebugLevel.INFO, string.Format("Received your UserID from server. Updating local value to: {0}", new object[1] { text4 }));
						}
					}
				}
				if (operationResponse.Parameters.ContainsKey(202))
				{
					...
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						DebugReturn(DebugLevel.INFO, string.Format("Received your NickName from server. Updating local value to: {0}", new object[1] { playername }));
					}
				}
		...
		case 227:
			...
			else if (operationResponse.ReturnCode != 0)
			{
				if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
				{
					Debug.LogWarning(string.Format("CreateRoom failed, client stays on masterserver: {0}.", new object[1] { operationResponse.ToStringFull() }));
				}
		...
		case 226:
			if (Server != ServerConnection.GameServer)
			{
				if (operationResponse.ReturnCode != 0)
				{
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						Debug.Log(string.Format("JoinRoom failed (room maybe closed by now). Client stays on masterserver: {0}. State: {1}", new object[2]
						{
							operationResponse.ToStringFull(),
							State
						}));
		...
		case 225:
			if (operationResponse.ReturnCode != 0)
			{
				...
				{
					Debug.LogWarning(string.Format("JoinRandom failed: {0}.", new object[1] { operationResponse.ToStringFull() }));
				}
				SendMonoMessage(PhotonNetworkingMessage.OnPhotonRandomJoinFailed, operationResponse.ReturnCode, operationResponse.DebugMessage);
			}
			else
		...
		case 219:
			...
			break;
		default:
			Debug.LogWarning(string.Format("OperationResponse unhandled: {0}", new object[1] { operationResponse.ToString() }));
			break;
		case 252:
		case 253:
			break;
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
		{
			Debug.Log(string.Format("OnStatusChanged: {0} current State: {1}", new object[2]
			{
				statusCode.ToString(),
				State
			}));
	...
	public void OnEvent(EventData photonEvent)
	{
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
		{
			Debug.Log(string.Format("OnEvent: {0}", new object[1] { photonEvent.ToString() }));
		}
		int num = -1;
	...
	protected internal void ExecuteRpc(ExitGames.Client.Photon.Hashtable rpcData, PhotonPlayer sender)
	{
		...
		if (photonView == null)
		{
			...
			if (flag)
			{
				Debug.LogWarning("Received RPC \"" + text + "\" for viewID " + num + " but this PhotonView does not exist! View was/is ours." + (flag2 ? " Owner called." : " Remote called.") + " By: " + sender.ID);
			}
			else
			{
				Debug.LogWarning("Received RPC \"" + text + "\" for viewID " + num + " but this PhotonView does not exist! Was remote PV." + (flag2 ? " Owner called." : " Remote called.") + " By: " + sender.ID + " Maybe GO was destroyed but RPC not cleaned up.");
			}
			return;
		}
			...
			for (int k = 0; k < value.Count; k++)
			{
				...
				if (cachedParemeters.Length == array2.Length)
				{
					if (CheckTypeMatch(cachedParemeters, array2))
					{
						...
						if (PhotonNetwork.StartRpcsAsCoroutine && (object)methodInfo.ReturnType == typeof(IEnumerator))
						{
				...
				{
					if (CheckTypeMatch(cachedParemeters, array2) && (object)cachedParemeters[cachedParemeters.Length - 1].ParameterType == typeof(PhotonMessageInfo))
					{
						num6++;
		...
		foreach (Type type2 in array2)
		{
			...
			text2 = (((object)type2 != null) ? (text2 + type2.Name) : (text2 + "null"));
		}
		if (num6 == 0)
		{
			if (num7 == 0)
	...
	private bool CheckTypeMatch(ParameterInfo[] methodParameters, Type[] callParameterTypes)
	{
		...
		{
			Type typeInfo = methodParameters[i].ParameterType;
			Type typeInfo2 = callParameterTypes[i];
			if ((object)callParameterTypes[i] != null && !typeInfo.IsAssignableFrom(typeInfo2) && (!typeInfo2.IsEnum || !Enum.GetUnderlyingType(typeInfo).IsAssignableFrom(typeInfo2)))
			{
				return false;
			}
		}
	...
	public void RegisterPhotonView(PhotonView netView)
	{
		...
		if (photonViewList.TryGetValue(netView.viewID, out value))
		{
			...
			Debug.LogError(string.Format("PhotonView ID duplicate found: {0}. New: {1} old: {2}. Maybe one wasn't destroyed on scene load?! Check for 'DontDestroyOnLoad'. Destroying old entry, adding new.", new object[3] { netView.viewID, netView, value }));
			RemoveInstantiatedGO(value.gameObject, true);
		}
	...
	internal void RPC(PhotonView view, string methodName, PhotonTargets target, PhotonPlayer player, bool encrypt, params object[] parameters)
	{
		...
		{
			RaiseEventOptions raiseEventOptions15 = new RaiseEventOptions();
			raiseEventOptions15.InterestGroup = view.group;
			raiseEventOptions15.Encrypt = encrypt;
			RaiseEventOptions raiseEventOptions16 = raiseEventOptions15;
			OpRaiseEvent(200, hashtable, true, raiseEventOptions16);
			ExecuteRpc(hashtable, LocalPlayer);
			break;
		}
		case PhotonTargets.Others:
		{
			RaiseEventOptions raiseEventOptions13 = new RaiseEventOptions();
			raiseEventOptions13.InterestGroup = view.group;
			raiseEventOptions13.Encrypt = encrypt;
			RaiseEventOptions raiseEventOptions14 = raiseEventOptions13;
			OpRaiseEvent(200, hashtable, true, raiseEventOptions14);
			break;
		}
		case PhotonTargets.AllBuffered:
		{
			RaiseEventOptions raiseEventOptions9 = new RaiseEventOptions();
			raiseEventOptions9.CachingOption = EventCaching.AddToRoomCache;
			raiseEventOptions9.Encrypt = encrypt;
			RaiseEventOptions raiseEventOptions10 = raiseEventOptions9;
		...
		case PhotonTargets.OthersBuffered:
		{
			...
			raiseEventOptions5.CachingOption = EventCaching.AddToRoomCache;
			raiseEventOptions5.Encrypt = encrypt;
			RaiseEventOptions raiseEventOptions6 = raiseEventOptions5;
		...
		case PhotonTargets.MasterClient:
		{
			...
			RaiseEventOptions raiseEventOptions11 = new RaiseEventOptions();
			raiseEventOptions11.Receivers = ReceiverGroup.MasterClient;
			raiseEventOptions11.Encrypt = encrypt;
			RaiseEventOptions raiseEventOptions12 = raiseEventOptions11;
			OpRaiseEvent(200, hashtable, true, raiseEventOptions12);
			break;
		}
		case PhotonTargets.AllViaServer:
		{
			RaiseEventOptions raiseEventOptions7 = new RaiseEventOptions();
			raiseEventOptions7.InterestGroup = view.group;
			raiseEventOptions7.Receivers = ReceiverGroup.All;
			raiseEventOptions7.Encrypt = encrypt;
			RaiseEventOptions raiseEventOptions8 = raiseEventOptions7;
		...
		case PhotonTargets.AllBufferedViaServer:
		{
			...
			raiseEventOptions3.InterestGroup = view.group;
			raiseEventOptions3.Receivers = ReceiverGroup.All;
			raiseEventOptions3.CachingOption = EventCaching.AddToRoomCache;
			raiseEventOptions3.Encrypt = encrypt;
			RaiseEventOptions raiseEventOptions4 = raiseEventOptions3;
