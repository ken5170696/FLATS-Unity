...
public static class PhotonNetwork
{
	...
	static PhotonNetwork()
	{
		...
		ConnectionProtocol protocol = PhotonServerSettings != null ? PhotonServerSettings.Protocol : ConnectionProtocol.Udp;
	...
	private static int AllocateViewID(int ownerId)
	{
		if (ownerId == 0)
		{
			...
			throw new Exception(string.Format("AllocateViewID() failed. Room (user {0}) is out of 'scene' viewIDs. It seems all available are in use.", new object[1] { ownerId }));
		}
		...
		throw new Exception(string.Format("AllocateViewID() failed. User {0} is out of subIds, as all viewIDs are used.", new object[1] { ownerId }));
	}
	...
	public static void UnAllocateViewID(int viewID)
	{
		...
		{
			UnityEngine.Debug.LogWarning(string.Format("UnAllocateViewID() should be called after the PhotonView was destroyed (GameObject.Destroy()). ViewID: {0} still found in: {1}", new object[2]
			{
				viewID,
				networkingPeer.photonViewList[viewID]
			}));
		}
	}
	...
	public static GameObject Instantiate(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
	{
		...
		Component[] array = value.GetPhotonViewsInChildren();
		int[] array2 = new int[array.Length];
		for (int i = 0; i < array2.Length; i++)
		{
			array2[i] = AllocateViewID(player.ID);
		}
		Hashtable evData = networkingPeer.SendInstantiate(prefabName, position, rotation, group, array2, data, false);
	...
	public static GameObject InstantiateSceneObject(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
	{
		...
		Component[] array = value.GetPhotonViewsInChildren();
		int[] array2 = AllocateSceneViewIDs(array.Length);
		if (array2 == null)
		{
		...
		Hashtable evData = networkingPeer.SendInstantiate(prefabName, position, rotation, group, array2, data, true);
		return networkingPeer.DoInstantiate(evData, networkingPeer.LocalPlayer, value);
	}
	...
	public static bool SetMasterClient(PhotonPlayer masterClientPlayer)
	{
		...
		if (room.serverSideMasterClient)
		{
			...
			hashtable2.Add((byte)248, networkingPeer.mMasterClientId);
	...
	[CompilerGenerated]
	private static int _003C_002Ecctor_003Eb__0()
	{
		return (int)startupStopwatch.ElapsedMilliseconds;
	}
