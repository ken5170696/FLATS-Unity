...
public class ETCInput : ETCSingleton<ETCInput>
{
	...
	public void RegisterControl(ETCBase ctrl)
	{
		...
		else if ((object)ctrl.GetType() == typeof(ETCButton))
		{
	...
	{
		if (ETCSingleton<ETCInput>.instance.controls.TryGetValue(ctrlName, out control))
		{
	...
	{
		if (ETCSingleton<ETCInput>.instance.axes.TryGetValue(axisName, out axis))
		{
			axis.axisValue = 0f;
	...
	{
		if (ETCSingleton<ETCInput>.instance.axes.TryGetValue(buttonName, out axis))
		{
	...
	public ETCInput()
	{
		axes = new Dictionary<string, ETCAxis>();
		controls = new Dictionary<string, ETCBase>();
