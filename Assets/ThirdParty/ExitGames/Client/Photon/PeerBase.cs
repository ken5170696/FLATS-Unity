	...
	public abstract class PeerBase
	{
		...
		protected internal bool IsIpv6
		{
			get
			{
				if (rt != null)
				{
					return rt.AddressResolvedAsIpv6;
				}
				return false;
			}
		}
		...
		internal string PepareWebSocketUrl(string serverAddress, string appId, object customData)
		{
			...
			if (customData != null && protocol.Serialize(customData) == null)
			{
		...
		protected PeerBase()
		{
			...
			lagRandomizer = new Random();
			NetSimListOutgoing = new LinkedList<SimulationItem>();
			NetSimListIncoming = new LinkedList<SimulationItem>();
			networkSimulationSettings = new NetworkSimulationSet();
			SerializeMemStream = new StreamBuffer();
