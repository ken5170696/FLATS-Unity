...
namespace ExitGames.Client.Photon
{
	internal class TPeer : PeerBase
	{
		...
		internal TPeer()
		{
			...
			pingRequest = new byte[5] { 240, 0, 0, 0, 0 };
			DoFraming = true;
		...
		internal override bool Connect(string serverAddress, string appID, object customData = null)
		{
			...
			if ((object)base.SocketImplementation != null)
			{
				try
				{
				...
				{
					base.Listener.DebugReturn(DebugLevel.ERROR, "CreateInstance() failed: " + ex);
				}
			}
			else
			{
				rt = new SocketTcpNetFxCore(this);
			}
			if (rt == null)
			{
