...
namespace ExitGames.Client.Photon.Chat
{
	public class ChatClient : IPhotonPeerListener
	{
		...
		{
			return string.Format("{0}:{1}", new object[2] { UserId, userName });
		}
		...
		void IPhotonPeerListener.OnOperationResponse(OperationResponse operationResponse)
		{
			...
			if (operationResponse.ReturnCode != 0 && (int)DebugOut >= 1)
			{
				...
				{
					listener.DebugReturn(DebugLevel.ERROR, string.Format("Chat Operation {0} unknown on server. Check your AppId and make sure it's for a Chat application.", new object[1] { operationResponse.OperationCode }));
					return;
				}
				listener.DebugReturn(DebugLevel.ERROR, string.Format("Chat Operation {0} failed (Code: {1}). Debug Message: {2}", new object[3] { operationResponse.OperationCode, operationResponse.ReturnCode, operationResponse.DebugMessage }));
			}
		}
		...
		private bool AuthenticateOnFrontEnd()
		{
			if (AuthValues != null)
			{
				if (AuthValues.Token == null || AuthValues.Token == "")
				{
					if ((int)DebugOut >= 1)
					{
