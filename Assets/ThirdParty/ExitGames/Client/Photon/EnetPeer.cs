...
namespace ExitGames.Client.Photon
{
	internal class EnetPeer : PeerBase
	{
		...
		internal EnetPeer()
		{
			sentReliableCommands = new List<NCommand>();
			windowSize = 128;
			channelArray = new EnetChannel[0];
			commandsToRemove = new Queue<int>();
			commandsToResend = new Queue<NCommand>();
		...
		internal override bool Connect(string ipport, string appID, object custom = null)
		{
			...
			InitPeerBase();
			rt = new SocketUdpNetFxCore(this);
			if (rt == null)
			{
		...
		private bool AreReliableCommandsInTransit()
		{
			lock (channelArray)
			{
				for (int i = 0; i < channelArray.Length; i++)
				{
					if (channelArray[i].outgoingReliableCommandsList.Count > 0)
					{
						return true;
					}
				}
		...
		internal int SerializeAckToBuffer()
		{
			...
			while (outgoingAcknowledgementsPool.Position + 20 <= outgoingAcknowledgementsPool.Length)
			{
				...
				int offset;
				Buffer.BlockCopy(outgoingAcknowledgementsPool.GetBufferAndAdvance(20, out offset), offset, udpBuffer, udpBufferIndex, 20);
				udpBufferIndex += 20;
				udpCommandCount++;
			}
		...
		{
			int offset;
			NCommand.CreateAck(outgoingAcknowledgementsPool.GetBufferAndAdvance(20, out offset), offset, readCommand, sendTime);
		}
		...
		internal override void ReceiveIncomingCommands(byte[] inBuff, int dataLength)
		{
				...
				else
				{
					...
					if (b == 204)
					{
						...
						if (value3 != (int)num)
						{
							...
							if (peerConnectionState != ConnectionStateValue.Disconnected && (int)base.debugOut >= 3)
							{
								EnqueueDebugReturn(DebugLevel.INFO, string.Format("Ignored package due to wrong CRC. Incoming:  {0:X} Local: {1:X}", new object[2]
								{
									(uint)value3,
									num
								}));
			...
			catch (Exception ex)
			{
				if ((int)base.debugOut >= 1)
				{
					EnqueueDebugReturn(DebugLevel.ERROR, string.Format("Exception while reading commands from incoming data: {0}", new object[1] { ex }));
				}
				SupportClass.WriteStackTrace(ex);
			}
		}
		...
		internal NCommand RemoveSentReliableCommand(int ackReceivedReliableSequenceNumber, int ackReceivedChannel)
		{
			...
			lock (sentReliableCommands)
			{
				...
				{
					EnqueueDebugReturn(DebugLevel.ALL, string.Format("No sent command for ACK (Ch: {0} Sq#: {1}). PeerState: {2}.", new object[3] { ackReceivedReliableSequenceNumber, ackReceivedChannel, peerConnectionState }));
				}
			}
			return nCommand;
		}
