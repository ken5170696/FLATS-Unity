	...
	public class Protocol16 : IProtocol
	{
		...
		private object DeserializeCustom(StreamBuffer din, byte customTypeCode)
		{
			...
			if (Protocol.CodeDict.TryGetValue(customTypeCode, out value))
			{
				...
				if ((int)(din.Position - position) != num)
				{
		...
		private GpType GetCodeOfType(Type type)
		{
			...
			if (type.IsEnum)
			{
				...
				if ((object)underlyingType == typeof(short))
				{
					return GpType.Short;
				}
			...
			if ((object)type == typeof(Hashtable))
			{
				return GpType.Hashtable;
			}
			if ((type.IsGenericType && !type.IsGenericTypeDefinition) && (object)typeof(Dictionary<, >) == type.GetGenericTypeDefinition())
			{
				return GpType.Dictionary;
			}
			if ((object)type == typeof(EventData))
			{
				return GpType.EventData;
			}
			if ((object)type == typeof(OperationRequest))
			{
				return GpType.OperationRequest;
			}
			if ((object)type == typeof(OperationResponse))
			{
		...
		{
			return Array.CreateInstance(GetTypeOfCode(arrayType), new int[1] { length });
		}
		...
		public override void Serialize(StreamBuffer dout, object serObject, bool setType)
		{
			...
			case GpType.Array:
				...
				else if ((object)serObject.GetType().GetElementType() == typeof(object))
				{
		...
		private void SerializeArray(StreamBuffer dout, Array serObject, bool setType)
		{
			...
			if (codeOfType != GpType.Unknown)
			{
				...
				if (codeOfType == GpType.Dictionary)
				{
					...
					{
						object value = serObject.GetValue(new int[1] { i });
				...
				else
				{
					...
					{
						object value2 = serObject.GetValue(new int[1] { j });
			...
			if (Protocol.TypeDict.TryGetValue(elementType, out value3))
			{
				...
				{
					object value4 = serObject.GetValue(new int[1] { k });
		...
		private void SerializeHashTable(StreamBuffer dout, Hashtable serObject, bool setType)
		{
			...
			{
				Serialize(dout, key, true);
				Serialize(dout, serObject[key], true);
			}
		}
		...
		{
			Type[] genericTypeArguments = dict.GetType().GetGenericArguments();
			setKeyType = (object)genericTypeArguments[0] == typeof(object);
			setValueType = (object)genericTypeArguments[1] == typeof(object);
			if (setKeyType)
			{
				writer.WriteByte(0);
			}
			...
			{
				GpType codeOfType = GetCodeOfType(genericTypeArguments[0]);
				if (codeOfType == GpType.Unknown || codeOfType == GpType.Dictionary)
				{
					throw new Exception("Unexpected - cannot serialize Dictionary with key type: " + genericTypeArguments[0]);
				}
			...
			GpType codeOfType2 = GetCodeOfType(genericTypeArguments[1]);
			if (codeOfType2 == GpType.Unknown)
			{
				throw new Exception("Unexpected - cannot serialize Dictionary with value type: " + genericTypeArguments[0]);
			}
			writer.WriteByte((byte)codeOfType2);
			if (codeOfType2 == GpType.Dictionary)
			{
				SerializeDictionaryHeader(writer, genericTypeArguments[1]);
			}
		}
		...
		private Array DeserializeArray(StreamBuffer din)
		{
			...
			case 121:
			{
				...
				array = Array.CreateInstance(array2.GetType(), new int[1] { num });
				array.SetValue(array2, new int[1]);
				for (short num3 = 1; num3 < num; num3++)
				{
					array2 = DeserializeArray(din);
					array.SetValue(array2, new int[1] { num3 });
				}
				break;
			}
			case 120:
			{
				array = Array.CreateInstance(typeof(byte[]), new int[1] { num });
				for (short num5 = 0; num5 < num; num5++)
				{
					Array value2 = DeserializeByteArray(din);
					array.SetValue(value2, new int[1] { num5 });
				}
				break;
			}
			case 99:
			{
				...
				{
					array = Array.CreateInstance(value.Type, new int[1] { num });
					for (int i = 0; i < num; i++)
					{
						...
						if (value.DeserializeStreamFunction == null)
						{
							...
							array.SetValue(value.DeserializeFunction(array3), new int[1] { i });
						}
						else
						{
							array.SetValue(value.DeserializeStreamFunction(din, num4), new int[1] { i });
						}
					}
					break;
				}
			...
			default:
			{
				...
				{
					array.SetValue(Deserialize(din, b), new int[1] { num2 });
				}
				break;
			}
			}
		...
		private bool DeserializeDictionaryArray(StreamBuffer din, short size, out Array arrayResult)
		{
			...
			arrayResult = Array.CreateInstance(type, new int[1] { size });
			for (short num = 0; num < size; num++)
			{
				...
				arrayResult.SetValue(dictionary, new int[1] { num });
			}
			return true;
		}
		...
		private Type DeserializeDictionaryType(StreamBuffer reader, out byte keyTypeCode, out byte valTypeCode)
		{
			...
			byte num = keyTypeCode;
			GpType gpType = (GpType)valTypeCode;
			Type type = ((num != 0) ? GetTypeOfCode(keyTypeCode) : typeof(object));
			Type type2 = ((gpType != GpType.Unknown) ? GetTypeOfCode(valTypeCode) : typeof(object));
			return typeof(Dictionary<, >).MakeGenericType(type, type2);
		}

		public Protocol16()
		{
			versionBytes = new byte[2] { 1, 6 };
			memShort = new byte[2];
			memLongBlock = new long[1];
			memLongBlockBytes = new byte[8];
			memDoubleBlock = new double[1];
			memDoubleBlockBytes = new byte[8];
			memInteger = new byte[4];
			memLong = new byte[8];
			memFloat = new byte[4];
			memDouble = new byte[8];
