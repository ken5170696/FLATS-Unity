...
public class MB3_TextureBaker : MB3_MeshBakerRoot
{
	...
	public IEnumerator CreateAtlasesCoroutine(ProgressUpdateDelegate progressInfo, CreateAtlasesCoroutineResult coroutineResult, bool saveAtlasesAsAssets = false, MB2_EditorMethodsInterface editorMethods = null, float maxTimePerFrame = 0.01f)
	{
		...
		for (int m = 0; m < OnCombinedTexturesCoroutineAtlasesAndRects.Length; m++)
		{
			OnCombinedTexturesCoroutineAtlasesAndRects[m] = new MB_AtlasesAndRects();
		}
		for (int n = 0; n < OnCombinedTexturesCoroutineAtlasesAndRects.Length; n++)
		{
			...
			if (_doMultiMaterial)
			{
				sourceMats = resultMaterials[n].sourceMaterials;
				resMatToPass = resultMaterials[n].combinedMaterial;
			}
			else
			{
				resMatToPass = _resultMaterial;
			}
			Debug.Log(string.Format("Creating atlases for result material {0} using shader {1}", new object[2] { resMatToPass, resMatToPass.shader }));
			...
			yield return combiner.CombineTexturesIntoAtlasesCoroutine(progressInfo, OnCombinedTexturesCoroutineAtlasesAndRects[n], resMatToPass, objsToMesh, sourceMats, editorMethods, coroutineResult2, maxTimePerFrame);
	...
	public MB3_TextureBaker()
	{
		...
		_packingAlgorithm = MB2_PackingAlgorithmEnum.MeshBakerTexturePacker;
		_meshBakerTexturePackerForcePowerOfTwo = true;
		_customShaderProperties = new List<ShaderTextureProperty>();
		_customShaderPropNames_Depricated = new List<string>();
		_considerNonTextureProperties = true;
		_doSuggestTreatment = true;
		resultMaterials = new MB_MultiMaterial[0];
