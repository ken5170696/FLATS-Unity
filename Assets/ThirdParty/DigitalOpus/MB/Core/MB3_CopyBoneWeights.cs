...
namespace DigitalOpus.MB.Core
{
	public class MB3_CopyBoneWeights
	{
		public static void CopyBoneWeightsFromSeamMeshToOtherMeshes(float radius, Mesh seamMesh, Mesh[] targetMeshes)
		{
			...
			Debug.Log(string.Format("The seam mesh has {0} vertices of which {1} are seam vertices.", new object[2]
			{
				seamMesh.vertices.Length,
				list.Count
			}));
			if (list.Count == 0)
			{
			...
			for (int j = 0; j < targetMeshes.Length; j++)
			{
				...
				{
					Debug.LogError(string.Format("Mesh {0} was null", new object[1] { j }));
					flag = true;
				}
				if (radius < 0f)
				{
			...
			for (int k = 0; k < targetMeshes.Length; k++)
			{
				...
				Debug.Log(string.Format("Copied boneweights for {1} vertices in mesh {0} that matched positions in the seam mesh.", new object[2]
				{
					targetMeshes[k].name,
					num
				}));
