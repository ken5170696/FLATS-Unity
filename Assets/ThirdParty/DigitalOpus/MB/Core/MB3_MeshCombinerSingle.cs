	...
	public class MB3_MeshCombinerSingle : MB3_MeshCombiner
	{
		...
		public class MeshChannelsCache
		{
			...
			internal MeshChannelsCache(MB3_MeshCombinerSingle mcs)
			{
				...
				_HALF_UV = new Vector2(0.5f, 0.5f);
		...
		[SerializeField]
		protected List<MB_DynamicGameObject> mbDynamicObjectsInCombinedMesh;
		...
		[SerializeField]
		protected Vector2[] uv2s;

		[SerializeField]
		protected Vector2[] uv3s;

		[SerializeField]
		protected Vector2[] uv4s;
		...
		private bool _addToCombined(GameObject[] goToAdd, int[] goToDelete, bool disableRendererInSource)
		{
			...
			int num6;
			for (num6 = 0; num6 < _goToAdd.Length; num6++)
			{
				if (!instance2Combined_MapContainsKey(_goToAdd[num6].GetInstanceID()) || Array.FindIndex(array, (int o) => o == _goToAdd[num6].GetInstanceID()) != -1)
				{
					MB_DynamicGameObject mB_DynamicGameObject = new MB_DynamicGameObject();
					GameObject gameObject = _goToAdd[num6];
					Material[] gOMaterials = MB_Utility.GetGOMaterials(gameObject);
					if (LOG_LEVEL >= MB2_LogLevel.trace)
					{
						Debug.Log(string.Format("Getting {0} shared materials for {1}", new object[2] { gOMaterials.Length, gameObject }));
					}
					if (gOMaterials == null)
					{
					...
					if (!(_goToAdd[num6] != null))
					{
						continue;
					}
					list.Add(mB_DynamicGameObject);
					mB_DynamicGameObject.name = string.Format("{0} {1}", new object[2]
					{
						_goToAdd[num6].ToString(),
						_goToAdd[num6].GetInstanceID()
					});
					mB_DynamicGameObject.instanceID = _goToAdd[num6].GetInstanceID();
				...
				else
				{
					if (LOG_LEVEL >= MB2_LogLevel.warn)
					{
						Debug.LogWarning("Object " + _goToAdd[num6].name + " has already been added");
					}
					_goToAdd[num6] = null;
				}
			}
			for (int num9 = 0; num9 < _goToAdd.Length; num9++)
			{
				if (_goToAdd[num9] != null && disableRendererInSource)
				{
					...
					if (LOG_LEVEL == MB2_LogLevel.trace)
					{
						Debug.Log("Disabling renderer on " + _goToAdd[num9].name + " id=" + _goToAdd[num9].GetInstanceID());
					}
				}
			}
		...
		private void _copyAndAdjustUVsFromMesh(MB_DynamicGameObject dgo, Mesh mesh, int vertsIdx, MeshChannelsCache meshChannelsCache)
		{
			...
			if (LOG_LEVEL >= MB2_LogLevel.trace)
			{
				Debug.Log(string.Format("_copyAndAdjustUVsFromMesh copied {0} verts", new object[1] { uv0Raw.Length }));
			}
		}
		...
		private void _updateGameObject(GameObject go, bool updateVertices, bool updateNormals, bool updateTangents, bool updateUV, bool updateUV2, bool updateUV3, bool updateUV4, bool updateColors, bool updateSkinningInfo, MeshChannelsCache meshChannelCache)
		{
			...
			float value = (localToWorldMatrix[1, 3] = num2);
		...
		// FLATS runtime glass rebuilds must never clear the serialized baked asset.
		public Mesh CreateRuntimeMesh()
		{
			if (!Application.isPlaying) throw new InvalidOperationException("Runtime mesh requires Play mode");
			_mesh = new Mesh { name = "FLATS runtime glass", hideFlags = HideFlags.DontSave };
			if (_targetRenderer is SkinnedMeshRenderer skinned) skinned.sharedMesh = _mesh;
			else if (_targetRenderer != null)
			{
		...
		{
			objectsInCombinedMesh = new List<GameObject>();
			lightmapIndex = -1;
			mbDynamicObjectsInCombinedMesh = new List<MB_DynamicGameObject>();
			_instance2combined_map = new Dictionary<int, MB_DynamicGameObject>();
			verts = new Vector3[0];
			normals = new Vector3[0];
			tangents = new Vector4[0];
			uvs = new Vector2[0];
