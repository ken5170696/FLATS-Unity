	...
	public class MB3_TextureCombiner 
	{
		...
		public class MatAndTransformToMerged
		{
			...
			public MatAndTransformToMerged(Material m)
			{
				...
				samplingRectMatAndUVTiling = default(DRect);
				materialTiling = default(DRect);
		...
		public class MB_TexSet
		{
			...
			{
				obUVoffset = new Vector2(0f, 0f);
				obUVscale = new Vector2(1f, 1f);
			...
			internal string GetDescription()
			{
				...
				{
					stringBuilder.AppendFormat("{0},", new object[1] { gos[i].name });
				}
				stringBuilder.AppendFormat("MATS=");
				for (int j = 0; j < mats.Count; j++)
				{
					stringBuilder.AppendFormat("{0},", new object[1] { mats[j].mat.name });
				}
			...
			internal string GetMatSubrectDescriptions()
			{
				...
				for (int i = 0; i < mats.Count; i++)
				{
					...
					{
						mats[i].mat.name,
						mats[i].samplingRectMatAndUVTiling
					});
				}
				return stringBuilder.ToString();
			}
		...
		private IEnumerator __Step1_CollectDistinctMatTexturesAndUsedObjects(ProgressUpdateDelegate progressInfo, CombineTexturesIntoAtlasesCoroutineResult result, List<GameObject> allObjsToMesh, List<Material> allowedMaterialsFilter, List<ShaderTextureProperty> texPropertyNames, MB2_EditorMethodsInterface textureEditorMethods, List<MB_TexSet> distinctMaterialTextures, List<GameObject> usedObjsToMesh)
		{
			...
			for (int i = 0; i < allObjsToMesh.Count; i++)
			{
				...
				for (int k = 0; k < gOMaterials.Length; k++)
				{
					if (progressInfo != null)
					{
						progressInfo(string.Format("Collecting textures for {0} submesh {1}", new object[2] { gameObject, k }), (float)i / (float)allObjsToMesh.Count / 2f);
					}
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				UnityEngine.Debug.Log(string.Format("Step1_CollectDistinctTextures collected {0} sets of textures fixOutOfBoundsUV={1} considerNonTextureProperties={2}", new object[3] { distinctMaterialTextures.Count, _fixOutOfBoundsUVs, _considerNonTextureProperties }));
			}
		...
		private IEnumerator __Step2_CalculateIdealSizesForTexturesInAtlasAndPadding(ProgressUpdateDelegate progressInfo, CombineTexturesIntoAtlasesCoroutineResult result, List<MB_TexSet> distinctMaterialTextures, List<ShaderTextureProperty> texPropertyNames, bool[] allTexturesAreNullAndSameColor, MB2_EditorMethodsInterface textureEditorMethods)
		{
			...
			for (int i = 0; i < texPropertyNames.Count; i++)
			{
				...
				{
					UnityEngine.Debug.Log(string.Format("AllTexturesAreNullAndSameColor prop: {0} val:{1}", new object[2]
					{
						texPropertyNames[i].name,
						allTexturesAreNullAndSameColor[i]
					}));
				}
			}
			...
			else
			{
				...
				for (int l = 0; l < distinctMaterialTextures.Count; l++)
				{
					...
					if (_resizePowerOfTwoTextures)
					{
						if (num2 <= num * 5)
						{
							UnityEngine.Debug.LogWarning(string.Format("Some of the textures have widths close to the size of the padding. It is not recommended to use _resizePowerOfTwoTextures with widths this small.", new object[1] { mB_TexSet.ToString() }));
						}
						if (num3 <= num * 5)
						{
							UnityEngine.Debug.LogWarning(string.Format("Some of the textures have heights close to the size of the padding. It is not recommended to use _resizePowerOfTwoTextures with heights this small.", new object[1] { mB_TexSet.ToString() }));
						}
				...
				for (int n = 0; n < distinctMaterialTextures.Count; n++)
				{
					for (int num4 = 0; num4 < texPropertyNames.Count; num4++)
					{
						...
						if (t != null && textureEditorMethods != null)
						{
							if (progressInfo != null)
							{
								progressInfo(string.Format("Convert texture {0} to readable format ", new object[1] { t }), 0.5f);
							}
				...
				for (int i = 0; i < distinctMaterialTextures.Count; i++)
				{
					...
					for (int j = 0; j < mB_TexSet.ts.Length; j++)
					{
						if (mB_TexSet.ts[j].t != null)
						{
							...
							{
								report.AppendFormat(" material scale {0} offset{1} ", new object[2]
								{
							...
							{
								report.AppendFormat(" obUV scale {0} offset{1} ", new object[2]
								{
									mB_TexSet.obUVscale.ToString("G4"),
				...
				for (int propIdx = 0; propIdx < numAtlases; propIdx++)
				{
					...
					else
					{
						...
						for (int texSetIdx = 0; texSetIdx < distinctMaterialTextures.Count; texSetIdx++)
						{
							...
							if (LOG_LEVEL >= MB2_LogLevel.trace)
							{
								UnityEngine.Debug.Log(string.Format("Adding texture {0} to atlas {1}", new object[2]
								{
									(texSet.ts[propIdx].t == null) ? "null" : texSet.ts[propIdx].t.ToString(),
		...
		private Rect[] __CreateAtlasesMBTexturePackerFast(ProgressUpdateDelegate progressInfo, int numAtlases, List<MB_TexSet> distinctMaterialTextures, List<ShaderTextureProperty> texPropertyNames, bool[] allTexturesAreNullAndSameColor, Material resultMaterial, Texture2D[] atlases, MB2_EditorMethodsInterface textureEditorMethods, int _padding)
		{
			...
			else
			{
				...
				array = mB2_TexturePacker.GetRects(list, num, _padding, out outW, out outH);
				if (LOG_LEVEL >= MB2_LogLevel.debug)
				{
		...
		private void MergeOverlappingDistinctMaterialTexturesAndCalcMaterialSubrects(List<MB_TexSet> distinctMaterialTextures, bool fixOutOfBoundsUVs)
		{
			...
			for (int i = 0; i < distinctMaterialTextures.Count; i++)
			{
				...
				if (LOG_LEVEL <= MB2_LogLevel.info)
				{
					UnityEngine.Debug.Log(string.Format("Textures in material(s) do not all use the same material tiling. This set of textures will not be considered for merge: {0} ", new object[1] { mB_TexSet.GetDescription() }));
				}
			...
			for (int m = 0; m < distinctMaterialTextures.Count; m++)
			{
				...
				for (int n = m + 1; n < distinctMaterialTextures.Count; n++)
				{
					...
					if (num3 < num4)
					{
						...
						if (LOG_LEVEL >= MB2_LogLevel.info)
						{
							...
							if (LOG_LEVEL >= MB2_LogLevel.trace)
							{
								for (int num9 = 0; num9 < mB_TexSet4.mats.Count; num9++)
								{
									...
									{
										mB_TexSet4.mats[num9].mat,
										mB_TexSet4.mats[num9].samplingRectMatAndUVTiling,
								...
								for (int num10 = 0; num10 < mB_TexSet3.mats.Count; num10++)
								{
									...
									{
										mB_TexSet3.mats[num10].mat,
										mB_TexSet3.mats[num10].samplingRectMatAndUVTiling,
						...
						if (LOG_LEVEL >= MB2_LogLevel.trace)
						{
							stringBuilder.AppendFormat("=== After Merge TextureSet {0}\n", new object[1] { mB_TexSet4.GetDescription() });
							for (int num14 = 0; num14 < mB_TexSet4.mats.Count; num14++)
							{
								stringBuilder.AppendFormat("tx1 Mat {0} matAndMeshUVRect {1} fullSamplingRect {2}\n", new object[3]
								{
									mB_TexSet4.mats[num14].mat,
									mB_TexSet4.mats[num14].samplingRectMatAndUVTiling,
									mB_TexSet4.ts[0].encapsulatingSamplingRect
								});
							}
					...
					if (LOG_LEVEL >= MB2_LogLevel.debug)
					{
						UnityEngine.Debug.Log(string.Format("Considered merging {0} and {1} but there was not enough overlap. It is more efficient to bake these to separate rectangles.", new object[2]
						{
							mB_TexSet4.GetDescription(),
							mB_TexSet3.GetDescription()
						}));
			...
			if (LOG_LEVEL >= MB2_LogLevel.info)
			{
				UnityEngine.Debug.Log(string.Format("MergeOverlappingDistinctMaterialTexturesAndCalcMaterialSubrects complete merged {0}", new object[1] { num }));
			}
		}
		...
		private Texture2D TintTextureWithTextureCombiner(Texture2D t, MB_TexSet sourceMaterial, ShaderTextureProperty shaderPropertyName)
		{
			if (LOG_LEVEL >= MB2_LogLevel.trace)
			{
				UnityEngine.Debug.Log(string.Format("Blending texture {0} mat {1} with non-texture properties using TextureBlender {2}", new object[3]
				{
					t.name,
					sourceMaterial.mats[0].mat,
		...
		public MB3_TextureCombiner()
		{
			...
			_atlasPadding = 1;
			_maxAtlasSize = 1;
			_maxTilingBakeSize = 1024;
			_meshBakerTexturePackerForcePowerOfTwo = true;
			_customShaderPropNames = new List<ShaderTextureProperty>();
			textureBlenders = new TextureBlender[0];
			_temporaryTextures = new List<Texture2D>();
