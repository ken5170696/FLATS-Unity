	...
	public class MB3_MultiMeshCombiner : MB3_MeshCombiner
	{
		...
		private bool _bakeStep1(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource)
		{
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				...
				{
					object obj = text;
					text = string.Concat(obj, " mesh", n, "(", meshCombiners[n].combinedMesh.GetObjectsInCombined().Count, ")\n");
				}
		...
		{
			obj2MeshCombinerMap = new Dictionary<int, CombinedMesh>();
			meshCombiners = new List<CombinedMesh>();
			_maxVertsInMesh = 65535;
