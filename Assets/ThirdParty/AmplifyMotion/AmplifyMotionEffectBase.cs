...
public class AmplifyMotionEffectBase : MonoBehaviour
{
	...
	private Dictionary<Camera, AmplifyMotionCamera> m_linkedCameras;
	...
	{
		int num = Mathf.Max(Mathf.FloorToInt(m_camera.pixelWidth + 0.5f), 1);
		int num2 = Mathf.Max(Mathf.FloorToInt(m_camera.pixelHeight + 0.5f), 1);
	...
	internal static bool CanRegister(GameObject gameObj, bool autoReg)
	{
		...
		if ((object)component.GetType() == typeof(SpriteRenderer))
		{
			return false;
		}
		...
		if (component.GetComponent<Cloth>() != null)
		{
			if (gameObj.GetComponent<Cloth>() != null)
			{
				return true;
			}
			Debug.LogWarning("[AmplifyMotion] Tearable cloth objects are not supported at this time. Ignoring cloth object \"" + component.name + "\"");
		}
		Type type = component.GetType();
		if ((object)type == typeof(MeshRenderer) || (object)type == typeof(SkinnedMeshRenderer))
		{
			return true;
		}
		return false;
	...
	public AmplifyMotionEffectBase()
	{
		...
		OverlayCameras = new Camera[0];
		CullingMask = -1;
		AutoRegisterObjs = true;
		MinResetDeltaDist = 1000f;
		ResetFrameDelay = 1;
		m_starting = true;
		m_linkedCameras = new Dictionary<Camera, AmplifyMotionCamera>();
		m_linkedCamerasChanged = true;
		m_globalObjectId = 1;
