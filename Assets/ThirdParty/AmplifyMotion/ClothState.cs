...
namespace AmplifyMotion
{
	internal class ClothState : MotionState
	{
		...
		internal override void Initialize()
		{
			...
			Mesh mesh = m_cloth.GetComponent<SkinnedMeshRenderer>().sharedMesh;
			...
			if (m_cloth.vertices.Length == mesh.vertices.Length)
			{
		...
		internal override void RenderVectors(Camera camera, float scale, Quality quality)
		{
			...
			for (int j = 0; j < m_sharedMaterials.Length; j++)
			{
				...
				{
					m_owner.Instance.ClothVectorsMaterial.mainTexture = materialDesc.material.mainTexture;
					if (materialDesc.cutoff)
					{
						m_owner.Instance.ClothVectorsMaterial.SetFloat("_Cutoff", materialDesc.material.GetFloat("_Cutoff"));
					}
				}
				if (m_owner.Instance.ClothVectorsMaterial.SetPass(pass))
				{
					Graphics.DrawMeshNow(m_clonedMesh, Matrix4x4.identity, j);
				}
			}
		}
