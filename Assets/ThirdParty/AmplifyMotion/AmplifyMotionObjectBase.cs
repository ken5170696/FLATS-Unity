...
public class AmplifyMotionObjectBase : MonoBehaviour
{
	...
	[SerializeField]
	protected bool m_applyToChildren;

	private AmplifyMotion.ObjectType m_type;

	private Dictionary<Camera, MotionState> m_states;
	...
	public AmplifyMotion.ObjectType Type
	{
		get
		{
			return m_type;
	...
	private bool InitializeType()
	{
		...
		if (AmplifyMotionEffectBase.CanRegister(base.gameObject, false) && component != null)
		{
			...
			else if (component.GetComponent<Cloth>() != null)
			{
	...
	internal void OnUpdateTransform(AmplifyMotionEffectBase inst, Camera camera, bool starting)
	{
		...
		if (m_states.TryGetValue(camera, out value) && !value.Error)
		{
			...
			value.UpdateTransform(starting || flag);
		}
		m_lastPosition = base.transform.position;
	}

	internal void OnRenderVectors(Camera camera, float scale, Quality quality)
	{
		MotionState value;
		if (m_states.TryGetValue(camera, out value) && !value.Error)
		{
			value.RenderVectors(camera, scale, quality);
	...
	public AmplifyMotionObjectBase()
	{
		...
		m_states = new Dictionary<Camera, MotionState>();
