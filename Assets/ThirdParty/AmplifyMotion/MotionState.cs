...
namespace AmplifyMotion
{
	...
	public abstract class MotionState 
	{
		...
		internal virtual void RenderVectors(Camera camera, float scale, Quality quality)
		{
		}
		...
		protected MaterialDesc[] ProcessSharedMaterials(Material[] mats)
		{
			...
			for (int i = 0; i < mats.Length; i++)
			{
				...
				bool flag = mats[i].GetTag("RenderType", false) == "TransparentCutout";
				bool flag2 = flag;
				array[i].coverage = mats[i].HasProperty("_MainTex") && flag2;
				array[i].cutoff = mats[i].HasProperty("_Cutoff");
				if (flag2 && !array[i].coverage && !m_materialWarnings.Contains(array[i].material))
				{
