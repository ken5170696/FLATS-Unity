...
namespace InControl
{
	public class UnityInputDeviceProfile : UnityInputDeviceProfileBase
	{
		...
		public override bool IsJoystick
		{
			get
			{
				if (LastResortRegex == null && (JoystickNames == null || JoystickNames.Length <= 0))
				{
					if (JoystickRegex != null)
					{
						return JoystickRegex.Length > 0;
					}
					return false;
				}
				return true;
		...
		public override bool IsSupportedOnThisPlatform
		{
			get
			{
				...
				{
					return base.IsSupportedOnThisPlatform;
				}
				return false;
			}
		}
		...
		private bool IsSupportedOnThisVersionOfUnity
		{
			get
			{
				...
				if (versionInfo >= MinUnityVersion)
				{
					return versionInfo <= MaxUnityVersion;
				}
				return false;
			}
		}
		...
		{
			if (!HasJoystickName(joystickName))
			{
				return HasLastResortRegex(joystickName);
			}
			return true;
		}
