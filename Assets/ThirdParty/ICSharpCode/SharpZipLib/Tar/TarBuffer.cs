...
namespace ICSharpCode.SharpZipLib.Tar
{
	public class TarBuffer
	{
		...
		public void WriteBlock(byte[] block)
		{
			...
			if (block.Length != 512)
			{
				string message = string.Format("TarBuffer.WriteBlock - block to write has length '{0}' which is not the block size of '{1}'", new object[2] { block.Length, 512 });
		...
		public void WriteBlock(byte[] buffer, int offset)
		{
			...
			{
				string message = string.Format("TarBuffer.WriteBlock - record has length '{0}' with offset '{1}' which is less than the record size of '{2}'", new object[3] { buffer.Length, offset, recordSize });
				throw new TarException(message);
			}
			if (currentBlockIndex >= BlockFactor)
			{
