	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public bool TestArchive(bool testData, TestStrategy strategy, ZipTestResultHandler resultHandler)
		{
			...
			try
			{
				...
				while (flag && num < Count)
				{
					...
					catch (ZipException ex)
					{
						...
						if (resultHandler != null)
						{
							resultHandler(testStatus, string.Format("Exception during test - '{0}'", new object[1] { ex.Message }));
						}
			...
			catch (Exception ex2)
			{
				...
				if (resultHandler != null)
				{
					resultHandler(testStatus, string.Format("Exception during test - '{0}'", new object[1] { ex2.Message }));
				}
			}
			if (resultHandler != null)
			{
		...
		private long TestLocalHeader(ZipEntry entry, HeaderTest tests)
		{
			lock (baseStream_)
			{
				...
				{
					throw new ZipException(string.Format("Wrong local header signature @{0:X}", new object[1] { offsetOfFirstEntry + entry.Offset }));
				}
				...
				if (flag2 && entry.IsFile)
				{
					...
					{
						throw new ZipException(string.Format("Version required to extract this entry not supported ({0})", new object[1] { num }));
					}
					if ((num2 & 0x3060) != 0)
					{
				...
				if (flag)
				{
					...
					{
						throw new ZipException(string.Format("Version required to extract this entry is invalid ({0})", new object[1] { num }));
					}
					if ((num2 & 0xC010) != 0)
					{
					...
					if ((num2 & 1) != 0 && num < 20)
					{
						throw new ZipException(string.Format("Version required to extract this entry is too low for encryption ({0})", new object[1] { num }));
					}
					if ((num2 & 0x40) != 0)
					{
						if ((num2 & 1) == 0)
					...
					{
						throw new ZipException(string.Format("Patched data requires higher version than ({0})", new object[1] { num }));
					}
					if (num2 != entry.Flags)
					{
				...
				if ((num2 & 8) == 0 || num8 > 0 || num7 > 0)
				{
					if (num8 != entry.Size)
					{
						throw new ZipException(string.Format("Size mismatch between central header({0}) and local header({1})", new object[2] { entry.Size, num8 }));
					}
					if (num7 != entry.CompressedSize && num7 != uint.MaxValue && num7 != -1)
					{
						throw new ZipException(string.Format("Compressed size mismatch between central header({0}) and local header({1})", new object[2] { entry.CompressedSize, num7 }));
					}
				}
		...
		private void CopyBytes(ZipUpdate update, Stream destination, Stream source, long bytesToCopy, bool updateCrc)
		{
			...
			if (num2 != num)
			{
				throw new ZipException(string.Format("Failed to copy bytes expected {0} read {1}", new object[2] { num, num2 }));
			}
			if (updateCrc)
			{
		...
		private void CopyEntryDataDirect(ZipUpdate update, Stream stream, bool updateCrc, ref long destinationPosition, ref long sourcePosition)
		{
			...
			if (num3 != num2)
			{
				throw new ZipException(string.Format("Failed to copy bytes expected {0} read {1}", new object[2] { num2, num3 }));
			}
			if (updateCrc)
			{
				update.OutEntry.Crc = crc.Value;
		...
		private void ReadEntries()
		{
			...
			if (num2 == ushort.MaxValue || num3 == ushort.MaxValue || num4 == 65535 || num5 == 65535 || num6 == uint.MaxValue || num7 == uint.MaxValue)
			{
				...
				if (num11 != 101075792)
				{
					throw new ZipException(string.Format("Invalid Zip64 Central directory signature at {0:X}", new object[1] { num10 }));
				}
				ReadLEUlong();
				ReadLEUshort();
				ReadLEUshort();
