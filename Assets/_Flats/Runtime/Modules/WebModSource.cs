...
namespace Flats.Modules
{
    ...
    public sealed class WebModSource : IModSource
    {
        ...
        [DllImport("__Internal")] static extern int FlatsModFetch(string url);
...
#endif
        async Task<byte[]> Get(string url,CancellationToken cancel)
        {
            ...
            int id=FlatsModFetch(uri.AbsoluteUri);
            try
            {
                int length;
        ...
        public Task Download(CatalogItem item,string destination,Action<long> progress,CancellationToken cancel)
        { throw new PlatformNotSupportedException("Use crosshair preset data in this browser; desktop packages require desktop FLATS."); }
    }
}
